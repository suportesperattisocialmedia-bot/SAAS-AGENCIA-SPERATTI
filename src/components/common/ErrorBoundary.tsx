    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-[300px] flex items-center justify-center p-6 w-full">
          <div className="max-w-md w-full bg-[#161618] border border-white/[0.06] rounded-[24px] p-6 text-center space-y-4 shadow-xl">
            ...
            <div>
              ...
              <p className="text-xs text-neutral-400 mt-1 tabular-nums break-words">
                {this.state.error?.message || 'Ocorreu uma falha inesperada na renderização da interface.'}
              </p>
            </div>
            <button
              onClick={this.handleReset}
              className="inline-flex items-center gap-2 px-3 py-1.5 bg-white/[0.06] hover:bg-white/[0.1] text-neutral-200 rounded-full text-xs font-medium transition-colors"
            >
